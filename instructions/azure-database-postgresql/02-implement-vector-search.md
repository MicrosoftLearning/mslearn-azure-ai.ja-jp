...
1. デプロイ スクリプトのメニューで「**2**」と入力すると、**[ベクトル拡張の構成許可リストを構成する]** オプションが起動します。 これにより、サーバーの **[azure.extensions]** 許可リストに **[vector]** 拡張が追加されるため、次のセクションで pgvector を有効にすることができます。 サーバーが再起動し、変更が適用されます。 **注:** 再起動が完了するまでに 1 〜 2 分かかることがあります。
