...
このセクションでは、デプロイ スクリプトに戻って PostgreSQL サーバーの接続情報を取得します。

1. **[Entra 認証を使用して PostgreSQL サーバーを作成する]** 操作が完了したら、「**2**」を入力して **[デプロイの状態を確認する]** オプションを起動します。 これで、サーバーの準備が整っていることが確認されます。

1. 「**3**」を入力して **[接続情報とアクセス トークンを取得する]** オプションを起動します。 これにより、必要な環境変数を含むファイルが作成されます。
...
1. 次のコマンドを実行して、前のステップで作成したファイルからターミナル セッションに環境変数を読み込みます。
    ...
    **PowerShell**
    ...
    >**注:** アクセス トークンは約 1 時間後に失効します。 後で再接続が必要な場合は、このスクリプトをもう一度実行し、オプション **3** を選択して新しいトークンを生成してから、もう一度変数をエクスポートしてください。
...
**psql の接続に失敗する**
- デプロイ スクリプトのオプション **3** を実行して *.env* と *.env.ps1* の両方のファイルが作成されたことを確認します
- **source .env** (Bash) または **. .\.env.ps1** (PowerShell) を実行して環境変数を読み込んだことを確認します
- アクセス トークンは約 1 時間後に失効するため、デプロイ スクリプトのオプション **3** をもう一度実行して新しいトークンを生成します
- デプロイ スクリプトのオプション **2** を実行して、サーバーの準備ができていることを確認します

**アクセス拒否または認証エラー**
- Microsoft Entra 管理者は、オプション **1** でサーバーが作成されると自動的に設定されます。 それでもアクセスが拒否される場合は、オプション **2** を実行して管理者を確認し、オプションがない場合はリソース グループを削除して、オプション **1** を再実行して再デプロイしてください。
