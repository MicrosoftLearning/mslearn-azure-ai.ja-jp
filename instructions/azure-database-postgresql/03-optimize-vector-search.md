...
1. **[Entra 認証を使用して PostgreSQL サーバーを作成する]** 操作が完了したら、「**2**」を入力して **[ベクトル拡張の構成許可リストを構成する]** オプションを起動します。 これにより、サーバーの **[azure.extensions]** 許可リストに **[vector]** 拡張が追加されるため、次のセクションで pgvector を有効にすることができます。 サーバーが再起動し、変更が適用されます。 **注:** 再起動が完了するまでに 1 〜 2 分かかることがあります。

1. 「**3**」と入力して、**[Check deployment status]** オプションを開始します。 これで、サーバーの準備が整っていることが確認されます。
...
**psql の接続に失敗する**
- デプロイ スクリプトのオプション **4** を実行して *.env* と *.env.ps1* の両方のファイルが作成されたことを確認します
...
**アクセス拒否または認証エラー**
- Microsoft Entra 管理者は、オプション **1** でサーバーが作成されると自動的に設定されます。 それでもアクセスが拒否される場合は、オプション **3** を実行して管理者を確認し、オプションがない場合はリソース グループを削除して、オプション **1** を再実行して再デプロイしてください。
