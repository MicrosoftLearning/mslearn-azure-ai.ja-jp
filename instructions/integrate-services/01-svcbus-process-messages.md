...
1. ターミナルで次のコマンドを実行して、アプリを起動します。 コマンドを実行する前に、演習の前半のコマンドを参照して、必要に応じて環境をアクティブ化してください。 *client* ディレクトリから移動した場合は、まず **cd client** を実行してください。
...
**環境変数を確認する**
- *.env* ファイルがプロジェクトのルートに存在し、**SERVICE_BUS_FQDN** 値が含まれているか確認します。
