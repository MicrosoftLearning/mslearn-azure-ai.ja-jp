...
1. 資格情報が設定された後、「**5**」と入力して **[アプリケーションを AKS にデプロイする]** オプションを起動します。 これにより、API が AKS クラスターにデプロイされます。

1. アプリがデプロイされた後、「**6**」と入力して **[デプロイの状態の確認]** オプションを起動します。 このオプションでは、各リソースが正常にデプロイされたかどうかが報告されます。

    すべてのサービスで**成功**メッセージが返された場合は、「**8**」と入力してデプロイ スクリプトを終了します。

    AKS クラスターが **[失敗]** または **[キャンセル済み]** の状態にある場合は、報告された問題を修正し、「**7**」と入力して **[失敗した AKS デプロイの削除]** オプションを起動してから、オプション **3** を再度実行します。 この保護されたオプションでは、正常な、または進行中のクラスターは削除されません。
...
- エラーで **Standard_D2s_v7** が利用不可である、または Azure リージョンが容量不足であると報告された場合は、オプション **8** で終了してください。 **AKS_VM_SIZE** をデプロイ スクリプトの上部近くに記載されている v5 または v6 のフォールバック サイズのいずれかに変更するか、**[場所]** の値を変更してからオプション **[3. AKS クラスターを作成する]** を再度実行します。
...
- オプション **6** で **[失敗]** または **[キャンセル済み]** が報告された場合は、根本的な問題を修正し、オプション **[7 失敗した AKS デプロイの削除]** を実行してからオプション **3** を再試行してください。 AKS リソースが作成されていない場合、削除は不要です。
...
**演習のリセット**
- トラブルシューティングのシナリオを最初からやり直す必要がある場合は、デプロイ スクリプトのオプション **[5. アプリケーションを AKS にデプロイする]** を実行して元の動作する構成を再デプロイしてください。
