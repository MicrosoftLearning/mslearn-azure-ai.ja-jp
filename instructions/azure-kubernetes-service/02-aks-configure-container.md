...
1. 次のコマンドを実行して、AKS と ACR をインストールするために必要なリソース プロバイダーがサブスクリプションにあることを確かめます。
...
1. 資格情報が構成された後、「**5**」と入力して **[Check deployment stats]** オプションを起動します。 このオプションでは、各リソースが正常にデプロイされたかどうかが報告されます。

    すべてのサービスで**成功**メッセージが返された場合は、「**6**」と入力してデプロイ スクリプトを終了します。
