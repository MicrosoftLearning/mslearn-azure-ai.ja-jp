...
1. ブラウザーを開き、次の URL を入力してスターター ファイルをダウンロードします。 ファイルは既定のダウンロード場所に保存されます。

    ```
    https://github.com/MicrosoftLearning/mslearn-azure-ai/raw/main/downloads/python/app-svc-container-python.zip
    ```

1. ファイルをコピーするか、プロジェクトに取り組みたいシステム内の場所に移動します。 その後、ファイルをフォルダーに解凍します。

1. Visual Studio Code (VS Code) を起動し、メニューで **[ファイル] > [フォルダーを開く...]** を選択してから、プロジェクト ファイルを含むフォルダーを選択します。

1. プロジェクトには Bash (*azdeploy.sh*) と PowerShell (*azdeploy.ps1*) の両方のデプロイ スクリプトが含まれています。 環境に適したファイルを開き、スクリプト上部の 2 つの値を必要に応じて変更し、変更を保存します。 **注:** スクリプトの他の部分は変更しないでください。
...
1. 前のステップで作成したファイルから環境変数をターミナル セッションに読み込む適切なコマンドを実行してください。
    ...
    **PowerShell**
    ...
    >**注:** ターミナルは開いたままにしてください。 閉じて新しいターミナルを作成すると、環境変数を再度作成するコマンドを実行しなければならない場合があります。
...
1. ブラウザーでこの URL を開きます。 ページ上部のメニューで、次の場所へ移動します。
    ...
    1. **[Bash]** は、ブラウザーベースのシェルとエクスプローラーを開きます。
    1. エクスプローラーで **/home/LogFiles/** に移動してログ ファイルを表示します。 フォルダー内のファイルを一覧表示するには `ls` と入力します。
...
1. VS Code ターミナルで次のコマンドを実行し、リソース グループとグループ内のすべてのリソースを削除します。 **\<rg-name>** を演習で先に選択した名前に置き換えます。 このコマンドにより、Azure でバックグラウンド タスクが起動し、リソース グループが削除されます。
...
- デプロイ スクリプトでは、詳細な **az acr build** 出力が抑制されます。 故障のトラブルシューティングを行うには、直近の ACR タスク実行のステータスとログを確認してください。
